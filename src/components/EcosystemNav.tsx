...
export default function EcosystemNav() {
  ...
  return (
    <div className="relative z-50">
      ...
      {/* Expanded Panel */}
      {isExpanded && (
        <div className="abs-card-brutal bg-black text-white w-80 max-h-96 overflow-y-auto absolute bottom-full mb-2">
            ...
            {ecosystemApps.map((app, index) => (
              <div key={index} className="relative">
                <a
                  ...
                  className={`block p-4 border-4 border-black transition-all duration-200 hover:transform hover:translate-y-1 text-black ${
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={`px-2 py-1 text-xs font-bold border-2 border-black ${getStatusColor(
                        app.status
                      )}`}
                    >
                  ...
                  {/* Description */}
                  <p className="text-xs font-mono mb-3 font-bold">
                    {app.description}
                  </p>
                ...
                {app.name === "IMPERFECTABS" && (
                  <div className="absolute -top-2 -left-2 h-6 w-6 bg-yellow-500 border-4 border-black transform rotate-45 flex items-center justify-center">
                    <span className="text-xs font-black transform -rotate-45">
                      📍
                    </span>
                  </div>
                )}
          ...
          <div className="mt-6 pt-4 border-t-4 border-white">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <div className="text-xs font-mono text-white">APPS</div>
              </div>
              <div>
                <div className="text-xl font-black text-lime-400">
                  {
                    Array.from(
                      new Set(ecosystemApps.flatMap((app) => app.networks))
                    ).length
                  }
                </div>
                <div className="text-xs font-mono text-white">NETWORKS</div>
              </div>
            </div>
          </div>
