...
const CONFIG = {
  ...
  contractAddress: "0x8087367f6fFC0fc4bFbd61d6Da4F8bde5bE27C3d",
...
class UserFlowTester {
  ...
  async simulateWorkoutSubmission() {
    ...
    // Sample workout data with coordinates for WeatherXM enhancement
    const workoutData = {
      reps: 25,
      formAccuracy: 85,
      streak: 15,
      duration: 120, // 2 minutes
      latitude: 40712800, // NYC latitude * 1e6 (40.7128)
      longitude: -74006000, // NYC longitude * 1e6 (-74.0060)
    };
    ...
    try {
      ...
      const tx = await this.contract.submitWorkoutSession(
        ...
        {
          gasLimit: 1000000, // Increased to handle potential gas requirements on Avalanche Fuji
        }
      );
      ...
      const aiRequestEvent = receipt.events?.find(
        (e) => e.event === "AIAnalysisRequested"
      );
  ...
  async monitorChainlinkResponse(requestId, timeoutMinutes = 5) {
    ...
    return new Promise((resolve) => {
      const checkInterval = setInterval(async () => {
        ...
        console.log(
          `🔍 Check ${attempts} (${Math.round(elapsed / 1000)}s elapsed)`
        );
        ...
        try {
          ...
          const completedFilter = this.contract.filters.AIAnalysisCompleted(
            this.wallet.address,
            requestId
          );
          ...
          if (completedEvents.length > 0) {
            ...
            console.log(
              "📊 Enhanced Score:",
              event.args.enhancedScore.toString()
            );
          ...
          const failedFilter = this.contract.filters.AIAnalysisFailed(
            this.wallet.address,
            requestId
          );
  ...
  async runFullTest() {
    ...
    try {
      ...
      if (submission.requestId) {
        const response = await this.monitorChainlinkResponse(
